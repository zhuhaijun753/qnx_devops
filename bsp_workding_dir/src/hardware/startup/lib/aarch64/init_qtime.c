...
static unsigned
timer_diff_v8gt(unsigned start)
{
	unsigned long	cnt;

	asm volatile("mrs %0, cntpct_el0" : "=r" (cnt));

	return (unsigned)(cnt - (unsigned long)start);
}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/hardware/startup/lib/aarch64/init_qtime.c $ $Rev: 900873 $")
#endif
