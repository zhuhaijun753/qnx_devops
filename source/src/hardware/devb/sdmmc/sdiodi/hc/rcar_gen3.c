...
const uint8_t h3_hs400_calib_code[2][CALIB_TABLE_MAX]= {
        { 0,0,0,0,0,1,1,2,3,4,5, 5, 6, 6, 7,11,15,16,16,17,17,17,17,17,18,18,18,18,19,20,21,21 },
        { 3,3,4,4,5,6,6,7,8,8,9, 9,10,11,12,15,16,16,17,17,17,17,17,18,18,18,18,19,20,21,22,22 }
};
...
const uint8_t m3w_v30_hs400_calib_code[2][CALIB_TABLE_MAX] = {
        { 0,0,0,0,2,3,4,4,5,6,7, 7, 8, 9, 9,10,11,12,13,15,16,17,17,18,19,19,20,21,21,22,23,23 },
        { 1,2,2,3,4,4,5,6,6,7,8, 9, 9,10,11,12,13,14,15,16,17,17,18,19,20,20,21,22,22,23,24,24 }
};
...
static int rcar_sdmmc_intr_event(sdio_hc_t *hc)
{
    ...
    if (cmd->flags & SCF_CTYPE_ADTC) {
        if ((stat1 & SDH_INFO1_RE) && (!(stat2 & SDH_INFO2_CMDE))) {
...
static void rcar_sdmmc_dma_start( sdio_hc_t *hc, sdio_cmd_t *cmd )
{
    ...
    sdmmc_write(sdmmc->vbase, MMC_SD_INFO1_MASK, sdmmc_read(sdmmc->vbase, MMC_SD_INFO1_MASK) & ~SDH_INFO1_AE);
    sdmmc_write(sdmmc->vbase, MMC_SD_INFO2_MASK, sdmmc_read(sdmmc->vbase, MMC_SD_INFO2_MASK) & ~SDH_INFO2_DATA_ERR);
...
static void rcar_sdmmc_dma_cmplt( sdio_hc_t *hc, sdio_cmd_t *cmd, int error)
{
    ...
    if(error) {
        sdmmc_write(sdmmc->vbase, MMC_DM_CM_RST, sdmmc_read(sdmmc->vbase, MMC_DM_CM_RST) & ~(DM_RST_DTRANRST1 | DM_RST_DTRANRST0));
        sdmmc_write(sdmmc->vbase, MMC_DM_CM_RST, sdmmc_read(sdmmc->vbase, MMC_DM_CM_RST) | (DM_RST_DTRANRST1 | DM_RST_DTRANRST0));
    }
...
static int rcar_sdmmc_cmd(sdio_hc_t *hc, sdio_cmd_t *cmd)
{
    ...
    sdmmc_write(sdmmc->vbase, MMC_SD_INFO2, SDH_INFO2_BMSK | (sdmmc_read(sdmmc->vbase, MMC_SD_INFO2) & ~SDH_INFO2_ALL_ERR));
    ...
    sdmmc_write(sdmmc->vbase, MMC_SD_INFO1_MASK, 0x0000FFFF & ~(SDH_INFO1_RE | SDH_INFO1_RMVL | SDH_INFO1_INST));

    sdmmc_write(sdmmc->vbase, MMC_SD_INFO2_MASK, 0x0000FFFF & ~(SDH_INFO2_CMD_ERR));
...
static int rcar_sdmmc_clk(sdio_hc_t *hc, int clk)
{
    ...
    if ((hc->flags & HC_FLAG_DEV_MMC) && (hc->caps & HC_CAP_HS400)) {
        if (hc->timing == TIMING_HS400) {
            /* Set SDH clock = 800MHz/400MHz, SD clock = 200MHz */
            ...
            /* Set SDH clock = 800MHz, SD clock = 200MHz */
            clkrate_val = 1;
        }
...
static int rcar_sdmmc_timing(sdio_hc_t *hc, int timing)
{
    ...
    /* This is for SDnCKCR setting also */
...
static int rcar_sdmmc_tune(sdio_hc_t *hc, int op)
{
    ...
    sdmmc_write(sdmmc->vbase, MMC_SD_INFO2, 0x0000 | SDH_INFO2_BMSK);
...
static int rcar_sdmmc_reset(sdio_hc_t *hc)
{
    ...
    sdmmc_write(sdmmc->vbase, MMC_SD_INFO1_MASK, 0x0001FFFF);  // all mask (Follow document ver.0027)
    sdmmc_write(sdmmc->vbase, MMC_SD_INFO2_MASK, 0x0000FFFF);  // all mask (Follow document ver.0027)
            ...
            case PRODUCT_ID_RCAR_H3:
                switch RCAR_GEN3_PRODUCT_REV(prr_val) {
                    ...
                    case PRODUCT_H3_REV_3_0:
                        ...
                        sdmmc->adjust_hs400_offset = MMC_SCC_TMPPORT3_OFFSET_3;
                        if (RCAR_GEN3_SDHI2_BASE == sdmmc->pbase) { // MMC0
                            sdmmc->adjust_hs400_calib_table = h3_hs400_calib_code[0];
                        } else if (RCAR_GEN3_SDHI3_BASE == sdmmc->pbase) { // MMC1
                            sdmmc->adjust_hs400_calib_table = h3_hs400_calib_code[1];
                        }
                        break;
                    default:
                        break;
...
//TODO: Add M3W 1.2 define:
                    //case PRODUCT_M3_REV_2_0:
                        /* Errata No. 109 - HS400 is specification limitation */
                        hc->caps &= ~HC_CAP_HS400;
                        break;
//TODO: Keep M3W 1.3 disabled until we can test HS400
                    ...
                    case PRODUCT_M3_REV_1_3:
                        ...
                        /* WA for Errata No. 123 (step1 - bad taps) */
                        sdmmc->hs400_bad_tap = (1 << 1) | (1 << 3);
                    ...
                    case PRODUCT_M3_REV_3_0:
                        ...
                        sdmmc->adjust_hs400_offset = MMC_SCC_TMPPORT3_OFFSET_3;
                        if (RCAR_GEN3_SDHI2_BASE == sdmmc->pbase) { // MMC0
                            sdmmc->adjust_hs400_calib_table = m3w_v30_hs400_calib_code[0];
                        } else if (RCAR_GEN3_SDHI3_BASE == sdmmc->pbase) { // MMC1
                            sdmmc->adjust_hs400_calib_table = m3w_v30_hs400_calib_code[1];
                        }
                        break;
                    default:
                        break;
...
#if 0
                /* UNTESTED: Since there are no prohibited TAP in E3, auto correction can be used */
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/hardware/devb/sdmmc/sdiodi/hc/rcar_gen3.c $ $Rev: 903416 $")
#endif
