...
#ifdef S32G_FLEXCAN
    /*  FlexCAN Clock Rate: 80 MHz, Sample-Point at: 87.5%  */
    /*                presdiv, propseg,   pseg1,   pseg2,  rjw */
    [CAN_BR_50K]  = {    0x63,    0x04,    0x07,    0x01,    1},
    [CAN_BR_125K] = {    0x27,    0x04,    0x07,    0x01,    0},
    [CAN_BR_250K] = {    0x13,    0x04,    0x07,    0x01,    1},
    [CAN_BR_500K] = {    0x09,    0x04,    0x07,    0x01,    1},
    [CAN_BR_1M]   = {    0x04,    0x04,    0x07,    0x01,    1},
#endif
#ifdef S32V_FLEXCAN
    /*  FlexCAN Clock Rate: 40 MHz, Sample-Point at: 87.5%  */
    /*                presdiv, propseg,   pseg1,   pseg2,  rjw */
    [CAN_BR_50K]  = {    0x31,    0x04,    0x07,    0x01,    1},
    [CAN_BR_125K] = {    0x13,    0x05,    0x06,    0x01,    0},
    [CAN_BR_250K] = {    0x09,    0x04,    0x07,    0x01,    1},
    [CAN_BR_500K] = {    0x04,    0x04,    0x07,    0x01,    1},
    [CAN_BR_1M]   = {    0x01,    0x07,    0x07,    0x02,    1},
#else
    /*  FlexCAN Clock Rate: 60 MHz, Sample-Point at: 87.5%   */
    /*                presdiv, propseg,   pseg1,   pseg2,  rjw */
...
void device_init(int argc, char *argv[])
{
    ...
    while(optind < argc)
    {
        ...
        while(optind < argc && *(optarg = argv[optind]) != '-')
        {
            ...
            else if(strncmp(optarg, "ringocan1", 9) == 0)
            {
            ...
            } else if(strncmp(optarg, "can", 3) == 0) {
                id = atoi(optarg+3);
                // Get the CANx interface parameters from HWINFO
                hwi_can = get_can_hwinfo(&can, id);

                if (0 == hwi_can) {
                    devinit.port = can.regbase;
                    devinit.mem = can.membase;
                    devinit.irqsys = can.irqvector;
                ...
                // Set default can unit number. Using CAN id if not set in command option.
                if(devinit.cinit.can_unit == -1) {
                    devinit.cinit.can_unit = id;
                }

                // Increment optarg
                optarg += 4; //sizeof("canX") - 1;
            } else {
...
void create_device(CANDEV_RINGO_INIT *devinit)
{
    ...
    if(!(devinit->flags & INIT_FLAGS_MDRIVER_INIT) || mdriver_intr == -1){
        ptr = (uint32_t *)devinfo->canmsg;
        for(i = 0; i < RINGO_CAN_MEM_SIZE_FLEXCAN / 4; i++, ptr++) {
            *ptr = 0;
        }
    }
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.0.0/trunk/hardware/can/mx6x/driver.c $ $Rev: 905826 $")
#endif
